...
Reusable GitHub Actions workflows can silently break for downstream users when Bioconductor container images are updated, upstream R packages change their APIs, or pinned action versions are deprecated. Because `bioconductor/workflows` is used by many repositories, a regression here has a multiplied blast radius.
...
- **`bioconductor/workflows` is now an R package**: Future maintainers should be aware that the `DESCRIPTION` file is a deliberate sentinel artifact, not the beginning of a real package. This should be documented clearly in the `DESCRIPTION` itself and the README.
