...
2. **Workflow Generator Synchronization**: Any changes made to `bioccheck.yml` inputs, secrets, or permissions MUST be mirrored in **both** the JavaScript object logic inside `docs/index.html` and the "Workflow Inputs & Parameters" reference table in `bioccheck_usage.md` to prevent synchronization drift (see ADR 0007).
